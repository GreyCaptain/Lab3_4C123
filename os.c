...
void SetInitialStack(int i){
  ...
  Stacks[i][STACKSIZE-16] = 0x04040404;  // R4
	...
	Stacks[i][STACKSIZE-1]  = 0x01000000;  // PSR with Thumb bit set to 1 
}
...
int OS_AddThreads(void(*thread0)(void),
  ...
  tcbs[1].next = &tcbs[2]; // TCB1 points to TCB2
	tcbs[2].next = &tcbs[3]; // TCB2 points to TCB3
	tcbs[3].next = &tcbs[4]; // TCB3 points to TCB4
	tcbs[4].next = &tcbs[5]; // TCB2 points to TCB5
	tcbs[5].next = &tcbs[0]; // TCB3 points to TCB0
...
void OS_Signal(int32_t *semaPt){
	...
	(*semaPt) = (*semaPt) + 1;
	if ((*semaPt) <=0){
		tpt = RunPt->next; //search for one blocked on this
...
int OS_FIFO_Put(uint32_t data){
	if(CurrentSize == FSIZE){
		LostData++;
		return -1; // full, return with error
	} 
	else{
		Fifo[PutI] = data; // Put
		PutI = (PutI+1)%FSIZE;
